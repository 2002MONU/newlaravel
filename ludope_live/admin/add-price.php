                    ...
                    <form action="" name="menu_form" method="post" enctype="multipart/form-data" accept-charset="utf-8">
                        ...
                        <div class="form-horizontal">
                            ...
                            <div class="box-body">
                                ...
                                <h4 class="page-title">👉🏻 Use Only this sheet <a href="price_sample.csv">Sample Excel Sheet</a></h4>
                                <h4 class="page-title">👉🏻 Header Should be Same </h4>
                                <h4 class="page-title">👉🏻 In Month Column Enter only : monthly  </h4>
                                ...
                                <!--div class="form-group">
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <input type="radio" id="board_type" name="board_type" value="2" required <?php if ($user['board_type'] == 2) {
    echo "checked";
} ?>> 2 Player
                                        <input type="radio" id="board_type" name="board_type" value="4" required <?php if ($user['board_type'] == 4) {
    echo "checked";
} ?>> 4 Player
...
if (isset($_POST['submit'])) {
    ...
    if ($_FILES['file']['name']) {
        ...
        if ($filename[1] == 'csv') {
            mysqli_query($conn, "delete from leader_board_price where board_type='$board_type'");
            ...
            while (($data = fgetcsv($handle, 10000, ",")) !== FALSE) {
                if (!$firstline) {
                    ...
                    if ($data[0] != '' && $data[1] != '') {
                        ...
                        $query = " INSERT into `leader_board_price` (`name`,`coins`,`type`,`date`,`board_type`) values ('$data[0]','$data[1]','$type','$date','$board_type')";
            ...
            fclose($handle);
        }echo "<script>document.location.href = 'leader_board_price.php?board_type=$board_type&msg=user_update'</script>";
        die;
    } else {
        echo "<script>document.location.href = 'leader_board_price.php?msg=error'</script>";
        die;
    }
}
?>
