...
//                                    }
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <form method="get" action="export_refferals_history.php" >
                                    <input type="hidden" name="page" value="<?php
//                                    if (isset($_GET['page'])) {
//                                        echo $_GET['page'];
//                                    } else {
...
//                                    }
                                    ...
                                    <div class="col-md-4">
                                        <input type="date" name="date1" id="form_date" class="form-control" placeholder="Form yyyy-mm-dd" required value="<?php
                    if (isset($_GET['date1'])) {
                        echo $_GET['date1'];
                    }
                    ?>"/>
                                     ...
                                     <div class="col-md-4">
                                         <input type="date" name="date2" id="to_date" class=" form-control" required placeholder="To yyyy-mm-dd" value="<?php
                    if (isset($_GET['date2'])) {
                        echo $_GET['date2'];
                    }
                    ?>"/>
                        ...
                        if (isset($_GET['user_search'])) {
                            ...
                            if ($_GET['referral_type'] == 'today') {
                                $sqlnew = mysqli_query($conn, "SELECT * FROM users WHERE refer_id>0 and (registerd BETWEEN '$today_start' AND '$today_end') ORDER BY id DESC LIMIT $start_from, $limit");
                            } else if ($_GET['referral_type'] == 'this_month') {
                                $sqlnew = mysqli_query($conn, "SELECT * FROM users WHERE refer_id>0 and (registerd BETWEEN '$month_start' AND '$month_end') ORDER BY id DESC LIMIT $start_from, $limit");
                            } else {
                    ...
                    <th>CSS grade</th>
                                        ...
                                        <ul class="pagination">
                                            ...
                                            if ($currentPage > 1) {
                                                echo '<li><a href="?page=' . ($currentPage - 1) . '&referral_type=' . $_GET['referral_type'] . '&user_search=' . $user_search . '&date1=' . $date1 . '&date2=' . $date2 . '">Previous</a></li>';
                                            }
                                            ...
                                            for ($i = $start; $i <= $end; $i++) {
                                                if ($i == $currentPage) {
                                                    ...
                                                    echo '<li><a href="?page=' . $i . '&referral_type=' . $_GET['referral_type'] . '&user_search=' . $user_search . '&date1=' . $date1 . '&date2=' . $date2 . '">' . $i . '</a></li>';
                                                }
                                            }

                                            // Display "Next" link if not on the last page
                                            if ($currentPage < $totalPages) {
                                                echo '<li><a href="?page=' . ($currentPage + 1) . '&referral_type=' . $_GET['referral_type'] . '&user_search=' . $user_search . '&date1=' . $date1 . '&date2=' . $date2 . '">Next</a></li>';
                                            }
                                            ?>
                                        </ul>
                                    </div>
