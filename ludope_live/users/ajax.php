...
//echo "SELECT * FROM Users WHERE email='".$username."' or mobile='".$username."'  AND pass='".$password."'";
    ...
    $sql = mysqli_query($conn, "SELECT * FROM sub_admins WHERE mobile='" . $username . "' AND password='" . $password . "'");

    $row = mysqli_fetch_assoc($sql);
    if ($row) {

        $_SESSION['login_id'] = $row['id'];
        $_SESSION['roles'] = $row['pages'];
        $_SESSION['dashboard_roles'] = $row['view_roles'];
