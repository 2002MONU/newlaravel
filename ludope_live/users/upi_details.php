...
if (isset($_GET["approve_status"])) {
    ...
    if ($status != '') {
        ...
        $sqlr = "UPDATE `kyc` SET upi_status = '$status',reason_upi = '$reason',is_bank='no',bank_approve='$status' where kid='$userid'";
                                        ...
                                        <tbody>
                                            ...
                                            if (!empty($_GET)) {
                                                if ($_GET['mobile'] != '') {
                                                    ...
                                                    if ($uinfo['id'] > 0) {
                                                        $kycinfo = mysqli_query($conn, "select * from kyc where user_id='" . $uinfo['id'] . "' and is_upi='yes' order by kid DESC");
                                                    }
                                                }
                                                if ($_GET['date1'] != '' && $_GET['date2'] != '') {
                                                    ...
                                                    $kycinfo = mysqli_query($conn, "select * from kyc where bank_approve='$status' and is_upi='yes' order by kid DESC");
                                                }
                                            } else {
                                                $status = 'pending';
