...
class SiteSettingController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'product_image' => 'image|mimes:jpeg,jpg,png|max:5120',
            'product_video' => 'file|mimes:mp4,mov,avi,flv|max:20480',
        ]);

        $data = SiteSetting::find($id);
        if ($request->hasFile('product_image')) {
            $img_extension = $request->file('product_image')->getClientOriginalExtension();
            $name = time() . '.' . $img_extension;
            $pathimage = $request->file('product_image')->move(public_path('admin/siteImage/og-image'), $name);
            $data->product_image = $name;
        }

        if ($request->hasFile('product_video')) {
            $img_extension = $request->file('product_video')->getClientOriginalExtension();
            $name = (time() + 1) . '.' . $img_extension;
            $pathimage = $request->file('product_video')->move(public_path('admin/siteImage/og-image'), $name);
            $data->product_video = $name;
        }
        ...
        $data->contact_information = $request->input('contact_information');
